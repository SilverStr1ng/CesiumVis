...
class GlobalVolumetricClouds {
  ...
  update() {
    ...
    const cartesian = this.viewer.scene.globe.ellipsoid.cartographicToCartesian(
  ...
  createGUI() {
    ...
    folder
      ...
      .onChange(() => {
        if (this._cloudsPostProcessStage) {
          this._cloudsPostProcessStage.uniforms.maxViewingDistance =
            this._uniforms.maxViewingDistance;
        }
      });
